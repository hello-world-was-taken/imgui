...
//  2026-04-21: Added a Win32-specific implementation of ImGui_ImplGlfw_GetContentScaleXXXX functions for legacy
//  GLFW 3.2. 2026-03-25: Mouse cursor is properly restored if changed by user app/code while using
//  glfwSetInputMode(..., GLFW_CURSOR_DISABLED) or ImGuiConfigFlags_NoMouseCursorChange. Amend change from 2025-12-10.
//  2026-02-10: Try to set IMGUI_IMPL_GLFW_DISABLE_X11 / IMGUI_IMPL_GLFW_DISABLE_WAYLAND automatically if corresponding
//  headers are not accessible. (#9225) 2026-01-25: [Docking] Improve workarounds for cases where GLFW is unable to
//  provide any reliable monitor info. Preserve existing monitor list when none of the new one is valid. (#9195, #7902,
//  #5683) 2026-01-18: [Docking] Dynamically load X11 functions to avoid -lx11 linking requirement introduced on
//  2025-09-10. 2025-12-12: Added IMGUI_IMPL_GLFW_DISABLE_X11 / IMGUI_IMPL_GLFW_DISABLE_WAYLAND to forcefully disable
//  either. 2025-12-10: Avoid repeated glfwSetCursor()/glfwSetInputMode() calls when unnecessary. Lowers overhead for
//  very high framerates (e.g. 10k+ FPS). 2025-11-06: Lower minimum requirement to GLFW 3.0. Though a recent version e.g
//  GLFW 3.4 is highly recommended. 2025-09-18: Call platform_io.ClearPlatformHandlers() on shutdown. 2025-09-15:
//  Content Scales are always reported as 1.0 on Wayland. FramebufferScale are always reported as 1.0 on X11. (#8920,
//  #8921) 2025-09-10: [Docking] Improve multi-viewport behavior in tiling WMs on X11 via the
...
// This is merely a band-aid to make using GLFW 3.2 a little bit nicer, but prefer to use GLFW 3.3+ or the full correct
// functions from the Win32 backend.
...
static void ImGui_ImplGlfw_SetWindowFloating(ImGui_ImplGlfw_Data *bd, GLFWwindow *window)
{
    ...
    if (glfwGetPlatform() == GLFW_PLATFORM_X11)
    {
        ...
        bd->XChangeProperty(
            display, xwindow, wm_type, XA_ATOM, 32, PropModeReplace, (unsigned char *)&wm_type_dialog, 1);
